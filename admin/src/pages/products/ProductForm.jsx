...
export default function ProductForm() {
    ...
    if (isEdit) {
      api.get(`/products/${id}`).then(res => {
        ...
        setForm({
          ...
          sizesEnabled: p.sizesEnabled !== false,
          sku: p.sku || '',
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(form).forEach(([k, v]) => {
      if (k === 'sizes') {
        if (v.length === 0) {
          data.append('sizes', ''); // Send empty marker to clear sizes on server
        } else {
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6">
                <div className="flex items-center justify-between pb-2 border-b mb-4">
                  <h3 className="font-bold text-gray-900">Plus-Size Matrix (UK)</h3>
                  <button
                    type="button"
                    onClick={() => setForm(prev => ({ ...prev, sizesEnabled: !prev.sizesEnabled }))}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      form.sizesEnabled ? 'bg-black' : 'bg-gray-200'
                    }`}
                    style={{ WebkitTapHighlightColor: 'transparent' }}
                  >
                    <span
                      className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                        form.sizesEnabled ? 'translate-x-5' : 'translate-x-0'
                      }`}
                    />
                  </button>
                </div>
                ...
                {form.sizesEnabled ? (
                  ...
                  <p className="text-sm text-gray-500 text-center py-4 font-medium">
                    Plus-Size Matrix is disabled. This product will be sold without size selection.
                  </p>
                )}
              </div>
